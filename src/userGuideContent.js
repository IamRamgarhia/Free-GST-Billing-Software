...
export const GUIDE_CONTENT = [
  ...
  { type: 'note', text: 'TL;DR  —  Extract the download, double-click "Free GST Billing - WINDOWS" once and click Install. A desktop icon appears. Open it whenever you need the app. Your data lives in the _system\\data\\ folder inside the app folder — back that folder up and you\'ve backed up everything.' },
  { type: 'spacer' },
  ...
  { type: 'ol', items: [
    'Download the ZIP from github.com/IamRamgarhia/Free-GST-Billing-Software/releases/latest. Use the Releases page, not the green Code button — that one gives the source code without the built app.',
    'Right-click the ZIP → Extract All. Extract it before opening anything: if you double-click the launcher while still inside the ZIP, Windows unpacks only that one file and the app cannot start.',
    'Open the extracted folder and double-click "Free GST Billing - WINDOWS". Each launcher is named after the system it is for, so there is nothing to work out. A small window opens — click Install. It takes 1-3 minutes the first time and asks no questions.',
    'The app opens in your browser by itself at http://localhost:47371, and a "Free GST Billing" icon is waiting on your Desktop for next time.',
    'Optional: click "Install App" in the browser address bar to make it look like a normal Windows program.',
  ]},
  ...
  { type: 'p', text: 'The .hta launcher is Windows-only — the same ZIP has "Free GST Billing.command" for macOS and "Free GST Billing.sh" for Linux. Or run from source anywhere with Node.js 18+: git clone the repo, npm install, npm start. Open http://localhost:47371 in your browser.' },
  { type: 'spacer' },
  ...
  { type: 'ol', items: [
    ...
    'On the NEW computer: install the app fresh (extract the ZIP, open the launcher, click Install).',
  ...
  { type: 'kv', rows: [
    ...
    ['How do I update to a new version?', 'Settings → Check for Updates → Update Now, or the Update button in the launcher. Either one backs up your data first, then fetches the newest release. Your invoices and settings are not touched.'],
  ...
  { type: 'kv', rows: [
    ['"Cannot connect to server" / blank page', 'The local server is not running. Open the "Free GST Billing" shortcut on your Desktop and click Open App — it starts the server if it is not already up.'],
    ['The launcher says "some of the app files are missing"', 'You opened it from inside the ZIP. Windows unpacks just that one file to a temp folder, so the rest of the app really is not next to it. Close it, right-click the ZIP in Downloads, choose Extract All, and open the launcher from the extracted folder.'],
    ['Windows shows "Windows protected your PC" or my antivirus blocked the installer', 'Click "More info" → "Run anyway". Or right-click "Free GST Billing - WINDOWS" → Properties → Unblock. The full source is on GitHub if you want to inspect it.'],
    ['Installer says Node.js install failed', 'Install Node.js manually from nodejs.org (pick LTS), then open the launcher again and click Install.'],
